...
from poc.archivator_lib.limits import ceil_div, parity_plan, recovery_blocks
...
class GeometryTests(unittest.TestCase):
    def test_seventy_gib_supergroup_uses_four_mib_slices(self):
        limit = 256 * 1024**2 - 1
        members = {f'chunk-{n}': limit for n in range(280)}
        groups = [list(members)[start:start + 56] for start in range(0, 280, 56)]
        plan = parity_plan(members, limit, groups=groups, margin_percent=110)
        self.assertEqual(plan.slice_size, 4 * 1024**2)
        ...
        self.assertGreaterEqual(plan.blocks, ceil_div(sum(counts), 5))
        self.assertGreaterEqual(plan.blocks, ceil_div(max(counts) * 110, 100))
    ...
    def test_source_block_limit_inclusive_and_one_byte_over(self):
        ...
        self.assertEqual(parity_plan(members, 256 * 1024**2).slice_size, 4096)
        members['0'] += 1
        self.assertEqual(parity_plan(members, 256 * 1024**2).slice_size, 8192)

    def test_recovery_limit_can_require_growth_before_source_limit(self):
        limit = 256 * 1024**2
        plan = parity_plan({'one': 26214 * 4096}, limit)
        self.assertEqual((plan.slice_size, plan.blocks), (4096, 32768))
        self.assertEqual(parity_plan({'one': 26215 * 4096}, limit).slice_size, 8192)
        members = {str(n): 4096 * 4096 for n in range(8)}
        self.assertEqual(parity_plan(members, limit, protect_all=True).slice_size, 8192)

    def test_redundancy_counts_each_files_partial_block(self):
        members = {str(n): 4097 for n in range(10)}
        self.assertEqual(recovery_blocks(members, 4096), 4)
        self.assertEqual(parity_plan(members, 65535).blocks, 4)
    ...
    def test_metadata_and_media_caps_cannot_be_bypassed_by_growing_slices(self):
        ...
        with self.assertRaises(ArchiveError):
            parity_plan({'one': 20000}, 65535, max_set_bytes=25000)
        with self.assertRaises(ArchiveError):
    ...
    def test_recorded_plan_preserves_geometry_after_metadata_compresses(self):
        ...
        plan = parity_plan(upper, 65535)
        actual = {'data': 50000, 'metadata': 1000}
        replay = parity_plan(actual, 65535, record=plan.record())
        self.assertEqual(replay.record(), plan.record())
        self.assertLessEqual(replay.total_bytes, plan.total_bytes)
        with self.assertRaises(ArchiveError):
            parity_plan(actual, 65535, record={**plan.record(), 'blocks': 1, 'volumes': 1})
...
class EarlyClosureTests(ArchiveTest):
    ...
    def test_supergroup_closes_before_count_limit_and_parity_regenerates(self):
        ...
        self.assertTrue(all(len(record['datagroups']) == 1 for record in records))
